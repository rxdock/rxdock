...
double RbtConstraint::Score() const {
  if (m_atomList.empty()) {
...
RbtConstraintPtr rxdock::CreateConstraint(RbtCoord &c, double &t,
                                          std::string &n, bool bCount) {
  if (n == "Any") {
    if (bCount)
...
void rxdock::ReadConstraint(std::istream &ifile, RbtConstraintPtr &cnt,
                            bool bCount) {
  std::string n;
  RbtCoord c;
  double t;
...
void rxdock::ReadConstraintFromMoe(std::istream &ifile, RbtConstraintPtr &cnt,
                                   bool bCount) {
  std::string n, extra;
  double x, y, z, t;
  ifile >> n;
...
void rxdock::ReadConstraints(std::istream &ifile, RbtConstraintList &cl,
                             bool bCount) {
  char ch;
  bool moe = false;
  cl.clear();
...
void RbtHydroAromaticConstraint::AddAtomList(RbtModelPtr lig, bool bCheck) {
  m_atomList = GetAtomListWithPredicate(lig->GetAtomList(), isAtomLipophilic());
  m_atomList =
      GetAtomListWithPredicate(m_atomList, isHybridState_eq(RbtAtom::AROM));
  if (bCheck && (m_atomList.size() < counter)) {
    std::ostringstream ostr;
    ostr << "The ligand has only " << m_atomList.size()
