...
RbtDouble RbtMCStats::Variance() const {
  return _total2 / _steps - pow(Mean(), 2);
}
RbtDouble RbtMCStats::AccRate() const {
  return float(_accepted) / float(_steps);
}
...
void RbtSimAnnTransform::MC(RbtDouble t, RbtInt blockLen, RbtDouble stepSize) {
  ...
  for (RbtInt iStep = 1; iStep <= blockLen; iStep++) {
    ...
    RbtBool bMetrop = ((delta < 0.0) || (exp(-1000.0 * delta / (8.314 * t)) >
                                         m_rand.GetRandom01()));
    // PASSED
    if (bMetrop) {
      score = newScore;
