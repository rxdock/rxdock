...
void RbtPolarIntraSF::ClearModel() {
  ...
  for (RbtInteractionListMapIter iter = m_intns.begin(); iter != m_intns.end();
       iter++) {
    (*iter).clear();
  }
  m_intns.clear();
  for (RbtInteractionListMapIter iter = m_prtIntns.begin();
  ...
  for (RbtInteractionCenterListIter iter = m_posList.begin();
       iter != m_posList.end(); iter++) {
    delete *iter;
  }
  m_posList.clear();
  for (RbtInteractionCenterListIter iter = m_negList.begin();
