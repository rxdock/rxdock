...
void RbtSFAgg::ScoreMap(RbtStringVariantMap &scoreMap) const {
  if (isEnabled()) {
    ...
    for (RbtBaseSFListConstIter iter = m_sf.begin(); iter != m_sf.end();
         iter++) {
...
RbtBaseSF *RbtSFAgg::GetSF(unsigned int iSF) const {
  if (iSF >= m_sf.size()) {
...
void RbtSFAgg::Unregister() {
  ...
  RbtBaseObject::Unregister(); // 7 Feb 2005 (DM) - unregister the agg, for the
