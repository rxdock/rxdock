...
void rxdock::to_json(json &j, const Model &model) {
  json atomList;
  for (const auto &aIter : model.m_atomList) {
    json atom = *aIter;
    atomList.push_back(atom);
  }

  json bondList;
  for (const auto &aIter : model.m_bondList) {
    json bond = *aIter;
    bondList.push_back(bond);
  }

  json atomListList;
  for (const auto &aListIter : model.m_ringList) {
    json atomList;
    for (const auto &aIter : aListIter) {
      json atom = *aIter;
      atomList.push_back(atom);
    }
    atomListList.push_back(atomList);
  }

  json pseudoAtomList;
  for (const auto &aIter : model.m_pseudoAtomList) {
    json atom = *aIter;
    pseudoAtomList.push_back(atom);
  }

  j = json{{"model-name", model.m_strName},
           {"title-list", model.m_titleList},
           {"atom-list", atomList},
           {"bond-list", bondList},
           {"segment-map", model.m_segmentMap},
           {"ring-list", atomListList},
           {"coord-names", model.m_coordNames},
           {"current-coord", model.m_currentCoord},
           {"data-map", model.m_dataMap},
           {"pseudo-atom-list", pseudoAtomList},
           {"model-mutator-pointer", *model.m_spMutator},
           // skipping FlexData pointer
           // skipping chromosome pointer
           {"ocupacy", model.m_occupancy},
           {"enabled", model.m_enabled}};
}

void rxdock::from_json(const json &j, Model &model) {
  j.at("model-name").get_to(model.m_strName);
  j.at("title-list").get_to(model.m_titleList);
  ...
  for (auto &bond : j.at("bond-list")) {
    ...
    model.m_bondList.push_back(spBond);
  }
  j.at("segment-map").get_to(model.m_segmentMap);
  ...
  j.at("coord-names").get_to(model.m_coordNames);
  j.at("current-coord").get_to(model.m_currentCoord);
  j.at("data-map").get_to(model.m_dataMap);

  for (auto &pAtom : j.at("pseudo-atom-list")) {
    AtomPtr sppAtom = AtomPtr(new Atom(pAtom));
    model.m_pseudoAtomList.push_back(sppAtom);
  }
  j.at("model-mutator-pointer").get_to(*model.m_spMutator);
  // skipping FlexData pointer
  // skipping chromosome pointer
  j.at("ocupacy").get_to(model.m_occupancy);
  j.at("enabled").get_to(model.m_enabled);
}
