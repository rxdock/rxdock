...
while (<PRMHANDLE>) {
    ...
    $psf_file = "$rbthome/" . $fields[1]
      if ( ( $fields[0] eq 'RECEPTOR_TOPOL_FILE' ) && ( $nfields > 1 ) );
    $crd_file = "$rbthome/" . $fields[1]
      if ( ( $fields[0] eq 'RECEPTOR_COORD_FILE' ) && ( $nfields > 1 ) );
    $mol2_file = "$rbthome/" . $fields[1]
      if ( ( $fields[0] eq 'RECEPTOR_FILE' ) && ( $nfields > 1 ) );
    chomp $psf_file;
    chomp $crd_file;
    $has_ph4 = 1
      if ( ( $nfields > 1 )
    ...
    $ph4_filename = "$rbthome/" . $fields[1]
      if ( ( $nfields > 1 )
        ...
        && $has_ph4 );
    $ph4_optfilename = "$rbthome/" . $fields[1]
      if ( ( $nfields > 1 ) && ( $fields[0] eq 'OPTIONAL_FILE' ) && $has_ph4 );
    $has_tether = 1
      if ( ( $nfields > 1 )
        && ( $fields[0] eq 'SECTION' )
        && ( $fields[1] eq 'tether' ) );
...
push( @src_files, "${ph4_optfilename}" )
  if ( ($has_ph4) && ( $ph4_optfilename ne "" ) );
...
do {
    $mode =
      get_input( "High Throughput (HT) or Exhaustive Docking (ED)", "HT" );
...
if ( $mode eq "ED" ) {
    ...
    $HT      = 1;
    $nStages = get_input(
        "Number of stages (excluding redocking, enter 0 for redocking only)",
        2 );
...
#Default score threshold for redocking is equal to target score for last docking stage
    ...
    if ( $nRedock > 0 ) {
        $redockTarget =
          get_input( "Enter target score for redocking", $redockTargetDef );
    }
    else {
...
print CMDHANDLE
"environment     = RBT_ROOT=$rbtroot;RBT_LIGDB=$rbtligdb;RBT_HOME=$rbthome;LD_LIBRARY_PATH=/usr/local/lib:$rbtroot/lib;\n";
...
print CLEANHANDLE
"echo All compressed SD files in ../sd/ will be concatenated into a single compressed file \${sdout}.gz\n";
...
print CLEANHANDLE
"echo You may now safely remove the ../sd/ directory if you wish to save disk space\n";
close CLEANHANDLE;
chmod 0755, $cleanFile;

print "To submit, cd $runRoot/tmp; condor_submit $condorFile\n";
print
"When all jobs have completed, run $runRoot/tmp/$cleanFile to concatenate the output files\n";
