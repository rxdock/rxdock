...
public:
  ...
  void SetNumImplicitHydrogens(const RbtUInt nHydrogens) {
  ...
  void SetFormalCharge(const RbtInt nFormalCharge) {
  ...
  void SetPartialCharge(const RbtDouble dPartialCharge) {
  ...
  void SetGroupCharge(const RbtDouble dGroupCharge) {
    m_dGroupCharge = dGroupCharge;
  }

  // AtomicMass
  RbtDouble GetAtomicMass() const { return m_dAtomicMass; }
  void SetAtomicMass(const RbtDouble dAtomicMass) {
    m_dAtomicMass = dAtomicMass;
  }
  ...
  void Translate(const RbtDouble vx, const RbtDouble vy, const RbtDouble vz) {
...
private:
  ...
  RbtBool m_bSelected; // Can be set/cleared by various search algorithms (e.g.
                       // FindRings)
...
// Note: atoms do not have to be bonded
inline RbtDouble BondDihedral(RbtAtom *pAtom1, RbtAtom *pAtom2, RbtAtom *pAtom3,
                              RbtAtom *pAtom4) {
...
class isAtomPtr_eq
    : public std::binary_function<RbtAtom *, RbtAtom *, RbtBool> {
public:
...
public:
  explicit isAtomInsideSphere(const RbtCoord &cc, RbtDouble rr)
      : c(cc), r2(rr * rr) {}
  RbtBool operator()(const RbtAtom *pAtom) const {
...
class isAtomNearCoordList : public std::unary_function<RbtAtom *, RbtBool> {
  ...
  RbtDouble r2; // radius squared (to avoid taking square roots)
public:
  explicit isAtomNearCoordList(const RbtCoordList &ccl, RbtDouble rr)
      : cl(ccl), r2(rr * rr) {}
  RbtBool operator()(const RbtAtom *pAtom) const;
};
...
inline RbtUInt GetNumAtomsWithAtomicNo_eq(const RbtAtomList &atomList,
                                          RbtInt nAtomicNo) {
...
RbtUInt GetNumMatchingAtoms(const RbtAtomList &atomList1,
...
void PrintQuantaCSDFormat(const RbtAtomList &atomList, ostream &s,
                          RbtInt nColor = 2, RbtInt nFormat = 0);
