...
public:
  ...
  void Rotate(const Quat &q) { FromQuat(q * ToQuat()); }
  void Rotate(const Vector &axis, double theta) { Rotate(Quat(axis, theta)); }

  // Insertion operator
  friend std::ostream &operator<<(std::ostream &s, const Euler &euler) {
